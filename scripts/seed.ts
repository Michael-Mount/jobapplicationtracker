...
import "@/lib/models";
import { Board, Column, JobApplicaiton } from "@/lib/models";

const USER_ID = "6a0622cd458cf5c8ea787a90";
  ...
  try {
    ...
    if (!board) {
      ...
      const { initializeUserBoard } =
        await import("../src/lib/init-user-board");
    ...
    const existingJobs = await JobApplicaiton.find({ userId: USER_ID });
    ...
    if (existingJobs.length > 0) {
      ...
      await JobApplicaiton.deleteMany({ userId: USER_ID });
    ...
    for (const [columnName, jobs] of Object.entries(jobsByColumn)) {
      ...
      if (!column) {
        console.warn(`⚠️  Could not find column object for "${columnName}"`);
        continue;
      }
